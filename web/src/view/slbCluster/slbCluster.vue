  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateSlbClusterFunc(scope.row)">变更</el-button>
      ...
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="RSA:"  prop="sshRsa" >
              <el-input v-model="formData.sshRsa" :clearable="false"  placeholder="请输入RSA" />
            </el-form-item>
            <el-form-item label="节点:"  prop="nodes" >
            ...
            <el-form-item label="证书路径:"  prop="pathSsl" >
              <el-input v-model="formData.pathSsl" :clearable="false"  placeholder="请输入证书路径" />
            </el-form-item>
            <el-form-item label="测试命令:"  prop="execTest" >
              <el-input v-model="formData.execTest" :clearable="false"  placeholder="请输入测试命令" />
            </el-form-item>
    ...
    <el-dialog v-model="detailShow" style="width: 800px" lock-scroll :before-close="closeDetailShow" title="查看详情" destroy-on-close>
      <el-scrollbar height="550px">
        <el-descriptions column="1" border>
                ...
                <el-descriptions-item label="测试命令">
                        {{ formData.execTest }}
                </el-descriptions-item>
...
const getTableData = async() => {
  const table = await getSlbClusterList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteSlbClusterByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSlbClusterFunc = async(row) => {
    const res = await findSlbCluster({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.reslbCluster
...
// 删除行
const deleteSlbClusterFunc = async (row) => {
    const res = await deleteSlbCluster({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createSlbCluster(formData.value)
                  break
                case 'update':
                  res = await updateSlbCluster(formData.value)
                  break
                default:
                  res = await createSlbCluster(formData.value)
                  break
