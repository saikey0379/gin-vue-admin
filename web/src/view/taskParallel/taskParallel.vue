  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateTaskParallelFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加':'修改'" destroy-on-close>
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="执行时间:"  prop="executedAt" >
              <el-date-picker v-model="formData.executedAt" type="date" style="width:100%" placeholder="选择日期" :clearable="false"  />
            </el-form-item>
          </el-form>
      </el-scrollbar>
      <template #footer>
      ...
      <el-scrollbar height="550px">
        <el-descriptions column="1" border>
                ...
                <el-descriptions-item label="执行时间">
                      {{ formatDate(formData.executedAt) }}
                </el-descriptions-item>
        </el-descriptions>
      </el-scrollbar>
    </el-dialog>
...
import {
  createTaskParallel,
  deleteTaskParallel,
  deleteTaskParallelByIds,
  updateTaskParallel,
  findTaskParallel,
  getTaskParallelList
} from '@/api/taskParallel'
...
const getTableData = async() => {
  const table = await getTaskParallelList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteTaskParallelByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateTaskParallelFunc = async(row) => {
    const res = await findTaskParallel({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.retaskParallel
...
// 删除行
const deleteTaskParallelFunc = async (row) => {
    const res = await deleteTaskParallel({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createTaskParallel(formData.value)
                  break
                case 'update':
                  res = await updateTaskParallel(formData.value)
                  break
                default:
                  res = await createTaskParallel(formData.value)
                  break
