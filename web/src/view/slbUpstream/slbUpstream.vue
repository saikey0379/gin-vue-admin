  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateSlbUpstreamFunc(scope.row)">变更</el-button>
      ...
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="自定义配置:"  prop="customize" >
              <el-input v-model="formData.customize" :clearable="false"  placeholder="请输入自定义配置" />
            </el-form-item>
            <el-form-item label="节点:"  prop="nodes" >
...
const getTableData = async() => {
  const table = await getSlbUpstreamList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteSlbUpstreamByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateSlbUpstreamFunc = async(row) => {
    const res = await findSlbUpstream({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteSlbUpstreamFunc = async (row) => {
    const res = await deleteSlbUpstream({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
