  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateTaskTimedFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getTaskTimedList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteTaskTimedByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateTaskTimedFunc = async(row) => {
    const res = await findTaskTimed({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.retaskTimed
...
// 删除行
const deleteTaskTimedFunc = async (row) => {
    const res = await deleteTaskTimed({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createTaskTimed(formData.value)
                  break
                case 'update':
                  res = await updateTaskTimed(formData.value)
                  break
                default:
                  res = await createTaskTimed(formData.value)
                  break
