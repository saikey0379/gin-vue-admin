  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateFileCommonFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getFileCommonList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteFileCommonByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateFileCommonFunc = async(row) => {
    const res = await findFileCommon({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.refileCommon
...
// 删除行
const deleteFileCommonFunc = async (row) => {
    const res = await deleteFileCommon({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const enterDialog = async () => {
     elFormRef.value?.validate( async (valid) => {
             if (!valid) return
              ...
              switch (type.value) {
                case 'create':
                  res = await createFileCommon(formData.value)
                  break
                case 'update':
                  res = await updateFileCommon(formData.value)
                  break
                default:
                  res = await createFileCommon(formData.value)
                  break
