<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="房间" prop="room_id">
             ...
             <el-input v-model.number="searchInfo.room_id" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="机柜" prop="name" width="120" />
        <el-table-column sortable align="left" label="标签" prop="label" width="120" />
        <el-table-column sortable align="left" label="房间" prop="room_name" width="120" />
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateIdcCabinetFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加':'修改'" destroy-on-close>
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            <el-form-item label="机柜:"  prop="name" >
              <el-input v-model="formData.name" :clearable="false"  placeholder="请输入机柜" />
            </el-form-item>
            <el-form-item label="标签:"  prop="label" >
...
const getIdcRoomsByIdcId = () => {
  ...
  getIdcRoomList({ IdcId: idc_id}).then(response => {
    ...
    console.error("调用房间列表查询接口失败:", error);
  });
};

// 监听IdcId变化，调用房间列表查询接口
watch(formData, () => {
  getIdcRoomsByIdcId();
});
...
const getTableData = async() => {
  const table = await getIdcCabinetList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteIdcCabinetByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateIdcCabinetFunc = async(row) => {
    const res = await findIdcCabinet({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteIdcCabinetFunc = async (row) => {
    const res = await deleteIdcCabinet({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
...
const openDialog = () => {
    type.value = 'create'
    dialogFormVisible.value = true
    isOptionRoomDisabled.value = true
}
