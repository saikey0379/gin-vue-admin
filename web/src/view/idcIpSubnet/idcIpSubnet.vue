<template>
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="网关" prop="gateway" width="120" :formatter="formatInfo" />
        ...
        <el-table-column align="left" label="VLAN" prop="vlanId" width="120" :formatter="formatInfo" />
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateIdcIpSubnetFunc(scope.row)">变更</el-button>
...
const getTableData = async() => {
  const table = await getIdcIpSubnetList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteIdcIpSubnetByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateIdcIpSubnetFunc = async(row) => {
    const res = await findIdcIpSubnet({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteIdcIpSubnetFunc = async (row) => {
    const res = await deleteIdcIpSubnet({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
