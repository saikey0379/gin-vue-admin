...
type FileResumeTransApi struct{}
...
func (b *FileResumeTransApi) ChunkContinue(c *gin.Context) {
	var fileDir = known.ReturnFileDir(c.Request.FormValue("fileType"), "")
	if fileDir == "" {
		message := "请指定目标路径!"
		global.GVA_LOG.Error(message, zap.Error(fmt.Errorf(message)))
		response.FailWithDetailed(exampleRes.FilePathResponse{FilePath: ""}, message, c)
		return
	}
	...
	file, err := fileResumeTransService.FindOrCreateFile(fileMd5, fileName, chunkTotal)
	if err != nil {
	...
	if err = fileResumeTransService.CreateFileChunk(file.ID, pathC, chunkNumber); err != nil {
...
func (b *FileResumeTransApi) ChunkFinish(c *gin.Context) {
	var fileDir = known.ReturnFileDir(c.Request.FormValue("fileType"), "")
	...
	fileMod, err := utils.StringToFileMod("0600")
	if err != nil {
	...
	filePath, err := utils.MakeFile(fileDir, fileName, fileMd5, fileMod)
	if err != nil {
...
func (b *FileResumeTransApi) ChunkRemove(c *gin.Context) {
	...
	err = fileResumeTransService.DeleteFileChunk(file.FileMd5, file.FilePath)
	if err != nil {
...
func (b *FileResumeTransApi) FindFile(c *gin.Context) {
	fileMd5 := c.Query("fileMd5")
	fileName := c.Query("fileName")
