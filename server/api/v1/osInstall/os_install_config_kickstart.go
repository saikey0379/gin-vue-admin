...
func (osInstallConfigKickstartApi *OsInstallConfigKickstartApi) GetContentBySn(c *gin.Context) {
	sn := c.Query("sn")
	if sn == "" {
	...
	redeviceBareMetal, err := service.ServiceGroupApp.DeviceServiceGroup.GetDeviceBareMetalBySn(sn)
	if err != nil {
		global.GVA_LOG.Error("查询失败!", zap.Error(err))
		response.FailWithMessage("查询失败", c)
		return
	...
	reosInstallConfigKickstart, err := osInstallConfigKickstartService.GetOsInstallConfigKickstart(uint(*redeviceBareMetal.KickstartId))
	if err != nil {
		global.GVA_LOG.Error("查询失败!", zap.Error(err))
		response.FailWithMessage("查询失败", c)
		return
	...
	content := strings.Replace(reosInstallConfigKickstart.Content, "\r\n", "\n", -1)

	response.ResultContent([]byte(content), c)
}
