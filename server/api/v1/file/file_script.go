...
func (fileScriptApi *FileScriptApi) CreateFileScript(c *gin.Context) {
	...
	byte_content := []byte(fileScript.Content)
	md5sum_content := md5.Sum(byte_content)
	fileScript.Md5 = hex.EncodeToString(md5sum_content[:])
	...
	fileScript.Manager = claimsReal.Username
...
func (fileScriptApi *FileScriptApi) GetContent(c *gin.Context) {
	name := c.Query("name")
	if name == "" {
	...
	if refileScript, err := fileScriptService.GetFileScriptByName(name); err != nil {
		global.GVA_LOG.Error("查询失败!", zap.Error(err))
		response.FailWithMessage("查询失败", c)
	} else {
		response.ResultContent([]byte(refileScript.Content), c)
	}
}
