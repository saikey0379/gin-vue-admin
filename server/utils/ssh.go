...
func (sc *SSHClient) RunShell(shell string) (output []byte, err error) {
...
func (sc *SSHClient) FileSync(syncsrc, dstdir, dstfile string) (output []byte, err error) {
	var command string
	if dstfile == "" {
	...
	err = cmd.Start()
	if err != nil {
		return out.Bytes(), fmt.Errorf(err.Error() + command)
	}
	err = cmd.Wait()
	if err != nil {
		return out.Bytes(), fmt.Errorf(err.Error() + command)
