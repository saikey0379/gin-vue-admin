    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加':'修改'" destroy-on-close>
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="PXE:"  prop="pxe_id" >
              <el-select v-model="formData.pxe_id" placeholder="请选择PXE" style="width:100%" :clearable="false" >
                <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
            <el-form-item label="Kickstart:"  prop="kickstart_id" >
              <el-select v-model="formData.kickstart_id" placeholder="请选择Kickstart" style="width:100%" :clearable="false" >
                <el-option v-for="(item,key) in intOptions" :key="key" :label="item.label" :value="item.value" />
              </el-select>
            </el-form-item>
...
const updateOsInstallQueueFunc = async(row) => {
    ...
    type.value = 'update'
    if (res.code === 0) {
        formData.value = res.data.requeue
