  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateDeviceBareMetalHardwareFunc(scope.row)">变更</el-button>
...
import {
  ...
  deleteDeviceBareMetalHardware,
  deleteDeviceBareMetalHardwareByIds,
  updateDeviceBareMetalHardware,
  findDeviceBareMetalHardware,
  getDeviceBareMetalHardwareList
...
const getTableData = async() => {
  const table = await getDeviceBareMetalHardwareList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteDeviceBareMetalHardwareFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteDeviceBareMetalHardwareByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateDeviceBareMetalHardwareFunc = async(row) => {
    const res = await findDeviceBareMetalHardware({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteDeviceBareMetalHardwareFunc = async (row) => {
    const res = await deleteDeviceBareMetalHardware({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
