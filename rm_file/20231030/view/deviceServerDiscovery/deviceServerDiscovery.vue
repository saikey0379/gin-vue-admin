  ...
  <div>
    ...
    <div class="gva-table-box">
        ...
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateDeviceServerDiscoveryFunc(scope.row)">变更</el-button>
      ...
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="生产商:"  prop="manufacturer" >
              <el-input v-model="formData.manufacturer" :clearable="false"  placeholder="请输入生产商" />
            </el-form-item>
...
import {
  ...
  deleteDeviceServerDiscovery,
  deleteDeviceServerDiscoveryByIds,
  updateDeviceServerDiscovery,
  findDeviceServerDiscovery,
  getDeviceServerDiscoveryList
...
const getTableData = async() => {
  const table = await getDeviceServerDiscoveryList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const deleteRow = (row) => {
    ElMessageBox.confirm('确定要删除吗?', '提示', {
        ...
        type: 'warning'
            ...
            deleteDeviceServerDiscoveryFunc(row)
        })
    }
...
const onDelete = async() => {
      ...
      const res = await deleteDeviceServerDiscoveryByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateDeviceServerDiscoveryFunc = async(row) => {
    const res = await findDeviceServerDiscovery({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteDeviceServerDiscoveryFunc = async (row) => {
    const res = await deleteDeviceServerDiscovery({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
