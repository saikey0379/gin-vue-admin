<template>
  <div>
    <div class="gva-search-box">
      ...
      <el-date-picker v-model="searchInfo.endCreatedAt" type="datetime" placeholder="结束日期" :disabled-date="time=> searchInfo.startCreatedAt ? time.getTime() < searchInfo.startCreatedAt.getTime() : false"></el-date-picker>
        ...
        <el-form-item label="网卡" prop="nic">
         <el-input v-model="searchInfo.nic" placeholder="搜索条件" />
        ...
        <el-form-item label="Agent版本" prop="agentVersion">
         <el-input v-model="searchInfo.agentVersion" placeholder="搜索条件" />
    ...
    <div class="gva-table-box">
        ...
        <el-table-column sortable align="left" label="Hostname" prop="hostname" width="120" />
        <el-table-column sortable align="left" label="IP" prop="ip" width="120" />
        ...
        <el-table-column sortable align="left" label="CPU" prop="cpuSum" width="120" />
        <el-table-column align="left" label="CPU信息" prop="cpu" width="120" />
        <el-table-column sortable align="left" label="内存" prop="memorySum" width="120" />
        <el-table-column align="left" label="内存信息" prop="memory" width="120" />
        <el-table-column align="left" label="网卡" prop="nic" width="120" />
        <el-table-column align="left" label="Gpu" prop="gpu" width="120" />
        <el-table-column align="left" label="主板" prop="motherboard" width="120" />
        <el-table-column align="left" label="Raid卡" prop="raid" width="120" />
        <el-table-column align="left" label="硬盘信息" prop="disk" width="120" />
        <el-table-column sortable align="left" label="Server类型" prop="serverType" width="120">
            <template #default="scope">
            {{ filterDict(scope.row.serverType,ServerTypeOptions) }}
            </template>
        </el-table-column>
        <el-table-column sortable align="left" label="Agent版本" prop="agentVersion" width="120" />
        <el-table-column align="left" label="操作">
            ...
            <el-button type="primary" link icon="edit" class="table-button" @click="updateServerInfoFunc(scope.row)">变更</el-button>
    ...
    <el-dialog v-model="dialogFormVisible" :before-close="closeDialog" :title="type==='create'?'添加':'修改'" destroy-on-close>
      <el-scrollbar height="500px">
          <el-form :model="formData" label-position="right" ref="elFormRef" :rules="rule" label-width="80px">
            ...
            <el-form-item label="型号:"  prop="model" >
              <el-input v-model="formData.model" :clearable="false"  placeholder="请输入型号" />
            </el-form-item>
            <el-form-item label="CPU:"  prop="cpuSum" >
            ...
            <el-form-item label="网卡:"  prop="nic" >
              <el-input v-model="formData.nic" :clearable="false"  placeholder="请输入网卡" />
            </el-form-item>
            <el-form-item label="Gpu:"  prop="gpu" >
            ...
            <el-form-item label="Agent版本:"  prop="agentVersion" >
              <el-input v-model="formData.agentVersion" :clearable="false"  placeholder="请输入Agent版本" />
            </el-form-item>
          </el-form>
      </el-scrollbar>
      <template #footer>
...
const getTableData = async() => {
  const table = await getServerInfoList({ page: page.value, pageSize: pageSize.value, ...searchInfo.value })
  if (table.code === 0) {
...
const onDelete = async() => {
      ...
      const res = await deleteServerInfoByIds({ ids })
      if (res.code === 0) {
        ElMessage({
          type: 'success',
          message: '删除成功'
...
// 更新行
const updateServerInfoFunc = async(row) => {
    const res = await findServerInfo({ ID: row.ID })
    type.value = 'update'
    if (res.code === 0) {
...
// 删除行
const deleteServerInfoFunc = async (row) => {
    const res = await deleteServerInfo({ ID: row.ID })
    if (res.code === 0) {
        ElMessage({
                type: 'success',
                message: '删除成功'
